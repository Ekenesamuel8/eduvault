...
- **Stellar CLI** (for contract deployment to testnet)
...
Confirm the active toolchain is `stable-x86_64-unknown-linux-gnu` (or the equivalent for your platform) with a recent date.

## WebAssembly Compilation Target

Soroban contracts compile to WebAssembly. Add the target used by the CI pipeline:
...
You should see `wasm32v1-none` in the list. The `soroban/build.sh` script uses `wasm32-unknown-unknown`; both targets produce valid WASM for Soroban, but CI validates against `wasm32v1-none`. If you use the build script, also add `wasm32-unknown-unknown`:
...
The Stellar CLI provides the `soroban` command used for contract deployment and testnet interaction. Install the version aligned with the workspace's `soroban-sdk` dependency:
...
The contracts live in the `soroban/` directory, which contains a Cargo workspace with two members:
